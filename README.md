...
| 전체 브랜치 | `git branch -a` | 로컬 + 원격 |
| 브랜치 생성 | `git branch <name>` | 브랜치 생성 |
| 브랜치 이동 | `git switch <name>` | 브랜치 전환(checkout도 가능!) |
| 생성+이동 | `git switch -c <name>` | 브랜치 생성 후 이동 |
| 브랜치 삭제 | `git branch -d <name>` | 병합 완료 브랜치 삭제 |
| 강제 삭제 | `git branch -D <name>` | 강제 삭제 |

## 원격 저장소(GitHub) 연동
| 목적 | 명령어 | 설명 |
| --- | --- | --- |
| 원격 확인 | `git remote -v` | 원격 주소 확인 |
| 원격 추가 | `git remote add origin <url>` | GitHub 연결 |
| 원격 수정 | `git remote set-url origin <url>` | 주소 변경 |
| 업로드 | `git push origin <branch>` | 원격 반영 |
| 최초 업로드 | `git push -u origin <branch>` | upstream 설정 |
| 내려받기 | `git pull` | 원격 변경 병합 |
| 변경만 수신 | `git fetch` | 병합 없이 수신 |
...
| 충돌 해결 표시 | `git add <file>` | 해결 완료 표시 |
| 병합 커밋 | `git commit` | 병합 완료 |

## 되돌리기 / 복구 (실수 대비)
| 목적 | 명령어 | 설명 |
| --- | --- | --- |
| 파일 복구 | `git restore <file>` | 수정 취소 |
| 스테이징 취소 | `git restore --staged <file>` | add 취소 |
| 커밋 되돌림 | `git revert <hash>` | 이력 유지 |
